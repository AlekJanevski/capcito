...
export default {
  ...
  mounted() {
	...
	watch: {
		// Handles the sorting while typing into the search input
    searchFilter () {
    ...
    filteredOptions () {
      ...
      for (const user of this.users) {
        if (this.searchFilter.length < 1 || user.name.match(regOption)){
          if (filtered.length < this.maxItem) filtered.push(user);
        }
      }
      return filtered;
    }
