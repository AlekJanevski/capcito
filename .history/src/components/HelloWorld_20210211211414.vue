				...
				<div class="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
					<div class="inline-block min-w-full overflow-hidden rounded-lg shadow">
						<table class="min-w-full leading-normal">
							...
							<tbody>
									<tr v-for="company in companyFilteredFeed" :key="company.id">
...
export default {
  name: 'Hello World',
	...
	setup () {
		...
		let companyList = ref([])
		let filter = ref("All")
		let companyNameSearchString = ref("")
    ...
    onMounted(() => {
			...
			// 	let companyNameSearchString = companyNameSearchString;

			// 	if (!companyNameSearchString){
			// 		console.log('ako ne search-am')
			// 		return companies.value;
			// 	}

			// 	companies.value = companies.value.filter(function(item) {
			// 		console.log('ako search-am')
			// 		if (item.name.value.indexOf(companyNameSearchString) !== -1 || item.name.value.toLowerCase().indexOf(companyNameSearchString) !== -1){
			// 			console.log('ako search-am vtor del')
			// 			return item.value;
			// 		}
			// 	})
