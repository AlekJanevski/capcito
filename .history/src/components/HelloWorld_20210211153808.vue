  ...
  mounted() {
	...
	computed: {
		...
		companyFilteredFeed: function () {
			...
			// searchString = companyNameSearchString.trim().toLowerCase();

			companies = companies.filter(function(companies){
				if (companies.name.indexOf(companyNameSearchString) !== -1 || companies.name.toLowerCase().indexOf(companyNameSearchString) !== -1){
					return companies;
				}
			})
