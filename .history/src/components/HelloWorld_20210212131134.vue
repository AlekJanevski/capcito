<template>
	<div class="container px-4 mx-auto sm:px-8">
		<div class="py-8">
			<!-- Heading starts-->
			<div>
				<h2 class="mb-5 text-2xl font-semibold leading-tight">Companies table(made with VueJS 3)</h2>
			</div>
			<!-- Heading ends -->
			...
			<div class="flex flex-col items-start justify-start my-2">
				...
				<div class="flex items-center justify-start">
					...
					<button
						...
						type="button"
						class="px-2 py-1 m-2 text-white transition duration-500 bg-indigo-500 border border-indigo-500 rounded-md select-none ease hover:bg-indigo-600 focus:outline-none focus:shadow-outline"
					>
						Ascending
					</button>
					<button
				...
				<div class="relative flex flex-col w-full mb-1 sm:mb-0 sm:flex-row">
					<div class="relative flex mr-3 w-60">
						<span class="flex items-center justify-start w-64 mr-3"> Filter by type: </span>
					...
					<div class="relative mt-3 sm:mt-0 sm:absolute sm:right-0">
						<!-- Search icon -->
...
export default {
  name: 'Hello World',
    ...
    onMounted(() => {
			...
			// Handle the company filtering by descending order
			function sortByDescending () {
				companyFilteredFeed.value.sort((a, b) => { return a.id - b.id; })
			}
			// Handle the company filtering by ascending order
			function sortByAscending () {
				companyFilteredFeed.value.sort((a, b) => { return b.id - a.id; })
			}

			// Computed
			// Returns the initial company list if the search is not active
			// Returns the company list using the search field value
